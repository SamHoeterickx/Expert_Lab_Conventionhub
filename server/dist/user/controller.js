"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteAccount = exports.updateUsername = exports.updatePassword = exports.logout = exports.getUser = exports.authenticateMe = exports.login = exports.register = void 0;
const model_1 = require("./model");
const register = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { email, password, repeatPassword, username } = req.body;
        const userData = { email, password, username };
        if (!email || !password || !repeatPassword || !username) {
            return res.status(422).send({
                status: 422,
                message: 'Missing register info'
            });
        }
        ;
        const checkUsernameAlreadyInUse = yield (0, model_1.isUserNameUnique)(username);
        if (checkUsernameAlreadyInUse) {
            return res.status(409).send({
                status: 409,
                message: 'Username is already in use'
            });
        }
        ;
        if (password.length < 8) {
            return res.status(400).send({
                status: 400,
                message: "Password isn't 8 characters long"
            });
        }
        //Check not supported passwords
        const excistingUser = yield (0, model_1.checkExcistingUser)(email);
        if (excistingUser) {
            return res.status(409).send({
                status: 409,
                message: 'Email already in use'
            });
        }
        ;
        if (password !== repeatPassword) {
            return res.status(401).send({
                status: 401,
                message: 'Passwords dont match'
            });
        }
        ;
        const newUser = yield (0, model_1.registerNewUser)(userData);
        if (!newUser) {
            return res.status(409).send({
                status: 409,
                message: 'Failed to create new user'
            });
        }
        ;
        res.cookie('session_id', newUser.id, {
            httpOnly: true,
            sameSite: 'strict',
            signed: true,
            maxAge: 7 * 24 * 60 * 60 * 1000
        });
        return res.status(201).send({
            status: 201,
            message: 'Register successfull'
        });
    }
    catch (error) {
        console.error('Error fetching register:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
});
exports.register = register;
const login = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { email, password } = req.body;
        if (!email || !password) {
            return res.status(422).send({
                status: 422,
                message: 'Missing login info'
            });
        }
        ;
        const excistingUser = yield (0, model_1.checkExcistingUser)(email);
        if (!excistingUser) {
            return res.status(401).send({
                status: 401,
                message: 'Invalid credentials'
            });
        }
        ;
        const passwordMatch = yield (0, model_1.verifyPassword)(password, excistingUser.password);
        if (!passwordMatch) {
            return res.status(401).send({
                status: 401,
                message: 'Invalid credentials'
            });
        }
        ;
        res.cookie('session_id', excistingUser.id, {
            httpOnly: true,
            sameSite: 'strict',
            signed: true,
            maxAge: 7 * 24 * 60 * 60 * 1000
        });
        return res.status(200).send({
            status: 200,
            message: 'Login succesfull',
            sessioId: excistingUser.id
        });
    }
    catch (error) {
        console.error('Error fetching login:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
});
exports.login = login;
const authenticateMe = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const sessionId = req.signedCookies.session_id;
        if (!sessionId) {
            return res.status(401).send({
                status: 401,
                message: 'Not authenticated'
            });
        }
        ;
        const user = yield (0, model_1.findUser)(sessionId);
        if (!user) {
            return res.status(401).send({
                status: 401,
                message: 'User not autenticated'
            });
        }
        ;
        return res.status(200).send({
            status: 200,
            message: 'Authentication successfull',
            data: user
        });
    }
    catch (error) {
        console.error('Error fetching authenticate:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.authenticateMe = authenticateMe;
const getUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.signedCookies.session_id;
        if (!userId) {
            return res.status(401).send({
                status: 401,
                message: 'Missing info'
            });
        }
        ;
        const userData = yield (0, model_1.getUserData)(userId);
        if (!userData) {
            return res.status(404).send({
                status: 404,
                message: 'User not found'
            });
        }
        ;
        return res.status(200).send({
            status: 200,
            message: 'User found successfull',
            data: userData
        });
    }
    catch (error) {
        console.error('Error fetching getUser:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.getUser = getUser;
const logout = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.signedCookies.session_id;
        if (!userId) {
            return res.status(401).send({
                status: 401,
                message: 'Missing info'
            });
        }
        ;
        res.cookie('session_id', '', { expires: new Date(0) });
        return res.status(200).send({
            status: 200,
            message: 'User found successfull',
        });
    }
    catch (error) {
        console.error('Error fetching logout:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.logout = logout;
const updatePassword = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { oldPassword, newPassword, repeatNewPassword, email } = req.body;
        const userId = req.signedCookies.session_id;
        if (!oldPassword || !newPassword || !repeatNewPassword || !userId || !email) {
            return res.status(401).send({
                status: 401,
                message: 'Missing credentials'
            });
        }
        if (newPassword !== repeatNewPassword) {
            return res.status(401).send({
                status: 401,
                message: 'newPasswords dont match'
            });
        }
        ;
        const excistingUser = yield (0, model_1.checkExcistingUser)(email);
        if (!excistingUser) {
            return res.status(404).send({
                status: 404,
                message: 'User not found'
            });
        }
        ;
        const passwordMatch = yield (0, model_1.verifyPassword)(oldPassword, excistingUser.password);
        if (!passwordMatch) {
            return res.status(401).send({
                status: 401,
                message: 'Incorrect old password'
            });
        }
        ;
        const updatedPassword = yield (0, model_1.updatePasswordById)(newPassword, userId);
        if (!updatedPassword) {
            return res.status(409).send({
                status: 409,
                message: 'Failed to update password'
            });
        }
        return res.status(200).send({
            status: 200,
            message: 'Password updated successfully'
        });
    }
    catch (error) {
        console.error('Error fetching update password:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.updatePassword = updatePassword;
const updateUsername = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { oldUsername, newUsername, email } = req.body;
        const userId = req.signedCookies.session_id;
        if (!oldUsername || !newUsername || !userId || !email) {
            return res.status(401).send({
                status: 401,
                message: 'Missing credentials'
            });
        }
        const checkUsernameAlreadyInUse = yield (0, model_1.isUserNameUnique)(newUsername);
        if (checkUsernameAlreadyInUse) {
            return res.status(409).send({
                status: 409,
                message: 'Username is already in use'
            });
        }
        ;
        const excistingUser = yield (0, model_1.checkExcistingUser)(email);
        if (!excistingUser) {
            return res.status(404).send({
                status: 404,
                message: 'User not found'
            });
        }
        ;
        if (oldUsername !== excistingUser.username) {
            return res.status(401).send({
                status: 401,
                message: 'Old username not correct'
            });
        }
        const updatedUsername = yield (0, model_1.updateUsernameById)(newUsername, userId);
        if (!updatedUsername) {
            return res.status(409).send({
                status: 409,
                message: 'Failed to update username'
            });
        }
        return res.status(200).send({
            status: 200,
            message: 'Username updated successfully'
        });
    }
    catch (error) {
        console.error('Error fetching update password:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.updateUsername = updateUsername;
const deleteAccount = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = req.signedCookies.session_id;
        // const { userId } = req.body;
        if (!userId) {
            return res.status(401).send({
                status: 401,
                message: 'Missing user info'
            });
        }
        const excistingUser = yield (0, model_1.findUser)(userId);
        if (!excistingUser) {
            return res.status(404).send({
                status: 404,
                message: 'User not found'
            });
        }
        ;
        const result = yield (0, model_1.deleteUserById)(userId);
        if (!result) {
            return res.status(409).send({
                status: 409,
                message: 'Failed to delete user'
            });
        }
        ;
        return res.status(204).send({
            status: 204,
            message: 'Account deleted successfully',
        });
    }
    catch (error) {
        console.error('Error fetching delete account:', error);
        return res.status(500).send({
            status: 500,
            message: error.message
        });
    }
    ;
});
exports.deleteAccount = deleteAccount;
